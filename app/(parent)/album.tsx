...
export default function ParentAlbumScreen() {
  ...
  const handleSaveWithNote = async () => {
    ...
    const connectionId = getConnectionId();
    if (!connectionId) {
      Alert.alert('Error', 'No connection found. Please connect first.');
      return;
    }
  ...
  const getUploaderInfo = (uploadedBy: string) => {
    if (uploadedBy === profile?.id) {
      return { name: 'Me', role: 'parent' as const };
    }
    return { name: partnerDisplayName, role: 'child' as const };
  };
            ...
            >
              <View style={styles.modalContent}>
                ...
                <TextInput
                  ...
                  placeholder="e.g., Made cookies today! 🍪"
